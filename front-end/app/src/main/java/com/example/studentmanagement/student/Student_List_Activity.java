...
public class Student_List_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<List<Student>> list_Student = apiInterface_listStudent.list_Student();
        list_Student.enqueue(new Callback<List<Student>>() {
            @Override
            public void onResponse(Call<List<Student>> call, Response<List<Student>> response) {
                if (response.isSuccessful()) {
                    studentList = response.body();
                    for (int i = 0; i < studentList.size(); i++) {
                        TableRow tableRow = new TableRow(Student_List_Activity.this);
                        tableRow.setPadding(10,0,10,0);

                        TextView list_studentID = new TextView(Student_List_Activity.this);
                        ...
                        TextView list_email = new TextView(Student_List_Activity.this);
                        list_email.setTextSize(20);
                        list_email.setGravity(Gravity.CENTER);
                        list_email.setBackgroundResource(R.drawable.border);

                        TextView list_phone = new TextView(Student_List_Activity.this);
            ...
            public void onFailure(Call<List<Student>> call, Throwable t) {
                Toast.makeText(Student_List_Activity.this,t.getLocalizedMessage(),Toast.LENGTH_SHORT).show();
